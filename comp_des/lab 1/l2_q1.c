...
const char *keywords[] = {"const", "bool", "char", "int","float", "double","unsigned", "return", "for", "while", "do", "switch","if", "else","case", "break", "continue"};
...
struct token getNextToken(FILE *f)
{
	...
	while(!gotToken && (c = fgetc(f))!=EOF)
	{
		...
		else if(c == '&' || c == '|')
		{
			...
			if(c == d)
			{
				tkn.lexeme[0] = tkn.lexeme[1] = c; tkn.lexeme[2] = '\0';
				tkn.row = row;
				tkn.col = col; 
				++col; 
				gotToken = 1;
